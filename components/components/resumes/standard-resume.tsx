      ...
      >
          ...
          <div className="relative z-10 flex flex-col md:flex-row gap-8 items-center md:items-start text-center md:text-left">
            ...
            >
              ...
              <Avatar className="h-32 w-32 md:h-40 md:w-40 border-4 border-background shadow-2xl relative">
                <AvatarImage src={RESUME_DATA.personalInfo.avatarUrl} alt="Profile picture" />
                <AvatarFallback className="text-4xl font-bold bg-primary/10 text-primary">{RESUME_DATA.personalInfo.initials}</AvatarFallback>
              </Avatar>
            </motion.div>
            ...
            <div className="flex-1 space-y-4">
              ...
              <div className="flex flex-wrap justify-center md:justify-start gap-3 pt-2">
                <Button variant="outline" size="sm" className="gap-2 h-9 rounded-full bg-background/50 backdrop-blur-sm hover:bg-primary/10 hover:text-primary hover:border-primary/50 transition-all duration-300">
            ...
            <div className="flex flex-col gap-3 min-w-[160px]">
              ...
              <div className="flex gap-2 justify-center">
                <Button variant="ghost" size="icon" className="h-10 w-10 text-muted-foreground hover:text-primary hover:bg-primary/10 rounded-full transition-all duration-300">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-0 lg:divide-x divide-border/60">
                ...
                {RESUME_DATA.experience.map((job, index) => (
                  <div key={index} className="relative pl-8 group">
                    ...
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-baseline mb-2">
                      <h3 className="font-bold text-lg text-foreground group-hover:text-primary transition-colors">{job.role}</h3>
                      <Badge variant="secondary" className="font-medium w-fit">{job.period}</Badge>
                    </div>
                    ...
                    <ul className="list-disc list-outside ml-4 space-y-2 text-sm text-muted-foreground/90 leading-relaxed">
                      {job.description.map((item, i) => (
                        <li key={i} dangerouslySetInnerHTML={{ __html: item.replace(/(Next\.js|Tailwind CSS|React)/g, '<span class="font-medium text-foreground">$1</span>') }} />
                      ))}
                    </ul>
                  </div>
                ))}
                    ...
                    <Card className="bg-gradient-to-br from-card to-muted/20 border-border/50 hover:border-primary/50 hover:shadow-lg transition-all duration-300">
                      <CardContent className="p-5">
                        <div className="flex justify-between items-start mb-3">
                          ...
                          <Badge variant={project.type === "Open Source" ? "default" : "outline"} className="text-[10px] uppercase tracking-wider font-bold shadow-sm">{project.type}</Badge>
                        </div>
          ...
          <div className="bg-muted/10 p-8 md:p-10 space-y-10 h-full">
            ...
            {/* Education */}
            <motion.section variants={itemVariants} aria-labelledby="education-heading">
              <div className="flex items-center gap-3 mb-5">
                <GraduationCap className="h-5 w-5 text-primary" />
                <h2 id="education-heading" className="text-lg font-bold text-foreground tracking-tight uppercase">
                  Education
                </h2>
              </div>
              <div className="space-y-6">
                {RESUME_DATA.education.map((edu, index) => (
                  <div key={index} className="relative pl-4 border-l-2 border-primary/20">
                    ...
                    <div className="text-sm text-primary font-medium mb-1">{edu.school}</div>
                    <div className="text-xs text-muted-foreground font-medium uppercase tracking-wider">{edu.year}</div>
                  </div>
                ))}
              </div>
            </motion.section>
            ...
            <motion.section variants={itemVariants} aria-labelledby="skills-heading">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {RESUME_DATA.skills.frontend.map((skill, i) => (
                      <Badge key={i} className="bg-background hover:bg-primary hover:text-primary-foreground text-foreground border-border transition-colors duration-300">{skill}</Badge>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-3 flex items-center gap-2">
                    <span className="w-1.5 h-1.5 rounded-full bg-primary" /> Tools
                  </h3>
                  <div className="flex flex-wrap gap-2">
            ...
            <motion.section variants={itemVariants} aria-labelledby="languages-heading">
              ...
              <div className="space-y-3">
                {RESUME_DATA.languages.map((lang, index) => (
                  <div key={index} className="flex justify-between items-center p-2 rounded-lg bg-background/50 border border-border/50">
